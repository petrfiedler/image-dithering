...
def reverseBits(a):
    ...
    # go from left to right and place the bits from right to left
    while shift:
        shift -= 1
...
def generateTresholdMap(n: int) -> np.ndarray:
    """ Generate Bayer threshold map with a given size.
        ...
        source: https://bisqwit.iki.fi/story/howto/dither/jy/
    """
    ...
    for y in range(n):
        for x in range(n):
            # upper part is shifted by -3 on the left and 1 on the right
            if y < n // 2:
